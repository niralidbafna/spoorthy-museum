...
export function ExhibitImage({ exhibit, onSelect, layoutIndex }: ExhibitImageProps) {
  ...
  return (
    ...
    >
      <div className="relative overflow-hidden rounded-none border border-muted/15 bg-white shadow-soft">
        <div className="relative h-[320px] sm:h-[420px] bg-[#f3efe8]">
          {exhibit.image ? (
            <Image
              src={exhibit.image}
              alt={exhibit.title}
              ...
              className={`object-cover transition duration-500 group-hover:scale-105 ${loaded ? 'opacity-100' : 'opacity-0'}`}
              onLoadingComplete={() => setLoaded(true)}
              priority={layoutIndex === 0}
            />
          ) : (
